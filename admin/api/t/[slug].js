...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const descripOG   = descripcion ? descripcion.slice(0, 120) : 'Descubre esta tienda en CashCach';
    const imagenOG    = imagen || 'https://appcashcash.com/og-default.png';
    const urlTienda   = `https://appcashcash.com/admin/tienda.html?id=${id}`;
...
<head>
  ...
  <meta property="og:url"         content="${escHtml(urlCorta)}">
  <meta property="og:title"       content="${escHtml(titulo)}">
  <meta property="og:description" content="${escHtml(descripOG)}">
  <meta property="og:image"       content="${escHtml(imagenOG)}">
  ...
  <meta name="twitter:description" content="${escHtml(descripOG)}">
  <meta name="twitter:image"       content="${escHtml(imagenOG)}">

  <!-- Redirigir inmediatamente -->
  <meta http-equiv="refresh" content="0;url=${escHtml(urlTienda)}">
  <style>
    body{font-family:sans-serif;display:flex;flex-direction:column;align-items:center;
         justify-content:center;min-height:100vh;margin:0;background:#f4f4f5}
    .logo{font-size:28px;font-weight:900;color:#1a8a7a;margin-bottom:12px}
...
<body>
  ...
  <p>Cargando tienda…</p>
  <script>window.location.replace(${JSON.stringify(urlTienda)});</script>
</body>
</html>`);
...
function paginaError() {
  ...
  <style>body{font-family:sans-serif;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#f4f4f5}.logo{font-size:28px;font-weight:900;color:#1a8a7a;margin-bottom:12px}p{color:#555}</style>
  </head><body><div class="logo">CashCach</div><p>Esta tienda no está disponible o el link ha cambiado.</p>
